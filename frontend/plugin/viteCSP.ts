  ...
  return {
    ...
    async transformIndexHtml(html: string, context: any) {
      ...
      if (configCsp.otherCsp) {
        for (const cspLabel of keys(configCsp.otherCsp))
          cspLine += joinCsp(cspLabel)
      }
      ...
      cspLine += await $('script').asyncForEach(
        `script-src ${joinMainSrc(configCsp.scriptSrc)}`,
      )
      await $('style').asyncForEach(
        `style-src ${joinMainSrc(configCsp.styleSrc)}`,
      )
