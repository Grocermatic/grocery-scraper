...
export const getProductInfoSection = async (
  ...
  let totalProduct = Number.POSITIVE_INFINITY
  for (
    let skipProduct = 0;
    skipProduct < totalProduct;
    skipProduct += paginationSize
  ) {
    ...
    if (totalProduct === Number.POSITIVE_INFINITY)
      totalProduct = JSON.parse(productJson).total
    await wait(1500)

    report.recordProductInfoPage(getProductInfoPage, productJson)
    const numProducts = report.get().productInfo.length
    console.debug(
      `IGA - ${sectionName} - ${numProducts} / ${totalProduct} products`,
    )
  }
  return report
}
