...
export const getWoolworthsProductInfo:GetProductInfo = (html) => {
  ...
  let priceString = $('div.primary[_ngcontent-serverapp-c372=""]').text()
  if (priceString === '') {
    const dollarString = $('span.price-dollars').text()
    const centString = $('span.price-cents').text()
    priceString = `${dollarString}.${centString}`
  }
  const price = getNumFromString(priceString)[0]

  let unitPriceCalc = getNumFromString($('span.price-per-cup').text())
  if (unitPriceCalc.length < 2) {
    unitPriceCalc = getNumFromString($('div.shelfProductTile-cupPrice').text())
  }
  const unitPrice = roundDecimal(unitPriceCalc[0] / unitPriceCalc[1], 2)
  const quantity = roundDecimal(price / unitPrice, 3)

  // Prefill mandatory values
  const productInfo:ProductInfo = {
    name: $('h1.shelfProductTile-title').first().text(),
  ...
  const servings = getNumFromString($('div[*ngif="productServingsPerPack"]').text())[0]
